...
**`check:i18n` reads every literal that can be the key, and counts the calls it cannot read (2026-09-13, review 2026-09-17).** The collecting regex wanted the literal right after `t(`, so `t(cond ? 'a' : 'b')` hid both keys — the thirteenth review counted twelve call sites with 21 keys, all present, none checked, and a fix of the twelfth review had just written the form once more. The script now reads the first argument of every `t(`/`mainT(` call to its end (brackets nest, strings and template literals are skipped whole) and takes each string literal standing where a value would: at the start, after `?`, after `:`. That covers both branches of a ternary and the values of `mainT({ … }[reason])` in `handlers.ts`, and leaves out the `'off'` in `state === 'off' ? …`. A call with no such literal is counted and the count printed; a bare `mainT()` in a comment is not a call. The two template literals that only chose between fixed names (`layoutEditor.tab${…}`, `layoutEditor.template${…}`) are ternaries of literals now. Measured: 1091 → 1110 renderer keys and 151 → 157 main-process keys, none missing; 67 renderer calls and 1 main-process call (the definition) left with a computed key. Counter-check with four keys deleted that were only used inside a ternary or an object (`projectImage.faviconMissing` and `layoutEditor.tabFrames` in both locales, `githubOff` and `buildDirIsHome` in `i18n.ts`): the old script reported no missing key, the new one all four. **Review 2026-09-18, finding 3:** that "1 main-process call" was the declaration `export function mainT(key: MainStringKey, …)`, whose parameter reads as a computed argument; the main process has no uncheckable call (an independent count over the TypeScript AST found 183 calls, the only computed one being the object map in `handlers.ts`, which the checker reads). A match preceded by `function` is skipped now: 67 and 0. Counter-check with two calls appended to `menu.ts` — `mainT(k as never)` and `mainT("gibtEsNicht")` — gave 1 computed and the missing key reported, then reverted.
