...
Damit sind beide Richtungen an echten Daten belegt: wo `filesAtHead` false ist, bleibt die Arbeit des Nutzers uncommittet; wo es true ist, nimmt der Amend das Lockfile mit. Was weiterhin **nicht** gemessen ist: ERESOLVE, ein echter Push unter Git-Sync, die Notiz über einen Restore oder ein Duplikat hinweg.
