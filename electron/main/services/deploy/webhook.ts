...
export const webhookAdapter: DeployAdapter = {
  ...
  async run(ctx: DeployContext): Promise<DeployResult> {
    if (ctx.target.destination.type !== 'webhook') throw new Error('Falscher Zieltyp für den Webhook-Adapter.')
    if (!ctx.secret) return { success: false, output: 'Für dieses Ziel ist keine Webhook-URL hinterlegt.' }

    let url: URL
    try {
      url = new URL(ctx.secret)
    } catch {
      return { success: false, output: 'Die hinterlegte Webhook-URL ist ungültig.' }
    }
    ...
    if (url.protocol !== 'https:') return { success: false, output: 'Webhooks werden nur über https ausgelöst.' }
    ...
    try {
      ...
      return { success: false, output: `POST ${url.origin} fehlgeschlagen.\n${reason}` }
    }
  }
}
