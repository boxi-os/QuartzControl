 ...
 * looked then.
 */
export function useSaveCommand(save: (() => void) | null): void {
  const current = useRef(save)
  current.current = save

  useEffect(() => {
    const run = (): void => current.current?.()
    handler = run
    return () => {
...
/** Runs the mounted page's save, if it registered one. Called by App.tsx's `app:command` listener. */
export function runSaveCommand(): void {
  handler?.()
}
