...
import { ChartColumn, EyeOff, Settings2 } from 'lucide-react'
import { Card, CardHeading, Field, Select, TextInput, Toggle } from '../../components/ui'
...
export default function SiteSettings({
  ...
  // Three cards, one per question the form answers. Inside a card a form's fields don't get
  // better by being stretched, so extra window width buys columns instead: single-column when
  // there isn't room, two from a laptop screen up, three on a wide one. Each field's line length
  // is capped by its grid track, not by a wrapper max-width.
  return (
    <>
      <Card>
        <CardHeading icon={Settings2} className="mb-3">{t('siteSettings.heading')}</CardHeading>
      ...
      <Card>
        <CardHeading icon={EyeOff} className="mb-3">{t('siteSettings.ignoreHeading')}</CardHeading>
        ...
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
          <Field label={t('siteSettings.ignorePatterns')} hint={t('siteSettings.ignorePatternsHint')} className="md:col-span-2">
            <textarea
              className="min-h-32 rounded-[7px] border border-ink/10 bg-surface px-2.5 py-1.5 font-mono text-ui text-text shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 dark:border-ink/10 dark:bg-ink/5"
...
function Analytics({
  ...
  return (
    <Card>
      <CardHeading icon={ChartColumn}>{t('siteSettings.analyticsHeading')}</CardHeading>
