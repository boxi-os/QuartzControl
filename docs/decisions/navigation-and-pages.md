...
- **The sidebar is grouped by what the user is doing, and a page is a job rather than a service.** Four groups (`ProjectLayout`'s `NAV_GROUPS`): Einrichtung (Konfiguration, Plugins), Gestaltung (Layout, Stile, Vorlagen), Veröffentlichung (Vorschau & Build, Git-Sync, Veröffentlichen), Wartung (Updates, Snapshots) - Übersicht stays ungrouped above them as the landing page. Screens that were one card or one form of their own became sub-tabs of the page they belong to: Content-Ordner and Übersetzungen are `?tab=` tabs of **Konfiguration** (both describe what the site *is* and where its texts come from - `ConfigEditor/ContentFolder.tsx`, `ConfigEditor/Localization.tsx`), Marktplatz is a tab of **Plugins**. Updates deliberately is *not*: it updates the Quartz core as much as the plugins, so it stays its own page next to Snapshots, where the other "keep this project healthy" work lives. `/content`, `/localization` and `/plugins/marketplace` stay as redirects to the right tab, so older links keep working - the same treatment `/themes` got. Every shell follows the Styles pattern (URL first, `useStickyState` fallback, one `PageHeader` for the page with a per-tab description), so a sub-tab renders no header of its own; that is also why `TAB_ICONS` has one entry per **sidebar item**, not per screen. A sub-tab that needs its own Save button puts it next to the controls it saves (`Localization`'s locale row) rather than reaching into the page header.
...
**A site without content/index.md has no page at its own address, and the app offers to write one — only from a dialog that names the folder (2026-09-13).** First reported by a beta tester: every preview of a project with copied or linked notes was Quartz's 404 page ("Diese Seite ist entweder nicht öffentlich oder existiert nicht."), an empty project with the example pages was fine. Read from the source, not guessed: Quartz's dev server serves `/` only when `public/index.html` exists (`cli/handlers.js` from line 516) and otherwise hands back `404.html`, and `@quartz-community/folder-page` makes a virtual page for every folder *except* the root — `getFolders(slug).filter((f) => f !== "." && f !== "tags")` and a matcher on `/index`, in the installed 0.1.0 and on GitHub `main` alike, with no option for it (`FolderPageOptions` has four keys, none about the root). So it is not a setting; of the three ways considered (a PR upstream, a page-type plugin of our own, writing the file) the user chose the third. `ContentStatus.hasIndex` is case-insensitive because Quartz lowercases every slug (measured: "Mein Ordner" became `./mein-ordner/`), and the hint stands in the three places the problem is met: the Content-Ordner card, the Übersicht's attention band, and Vorschau & Build. `content.createIndex({ projectPath, title })` asks for a title because without one `@quartz-community/note-properties` names the page after its file (`data.title = file.stem`), i.e. "index" — `aliases` would only add redirect addresses. The body is a list of the top-level folders and notes as markdown links in angle brackets (a wikilink cannot name a folder without an index.md), leaving out dotfiles and `ignorePatterns`, and the file is opened with `wx`, so it never overwrites. Measured in a real `quartz build` of a copy of `gui-test` with a folder containing a space, a linked folder, `Äpfel & Birnen.md`, `[klammer] Notiz.md`, `.obsidian`, `private` and `templates`: the generated list held the six expected entries, all six links resolved to a page that exists, `<title>` carried the quoted title, and a second call answered "already has an index.md" without touching the file. In the built app with a throwaway profile, once with a real folder and once with a linked one: all three hints present before, the dialog prefilled with the project name (and naming the vault path when linked), after "Startseite anlegen" the file in place — in the vault, link intact — the hints gone and the path announced.

**The start page's list asks Quartz's globby instead of imitating it (2026-09-13, reviews 2026-09-16 and 2026-09-17).** The list left out `ignorePatterns` by testing each top-level *name* with `matchesGlob`; Quartz applies them to the *path* under `content/`, so `private/**` linked a folder the build drops (twelfth review, finding 7). The fix tested a folder as `name`, `name/` and `name/x/y.md`, checked against Quartz's own globby with six patterns — and the thirteenth review, running 19 patterns over five entries against the same globby, still found 13 wrong answers out of 95: `y/*` and `y/*.md` drop a folder without subfolders (the probe sat a level too deep, so the list linked a page that does not exist); fast-glob only prunes a directory for a pattern whose last segment is static or which ends in `/**` (`isAffectDepthOfReadingPattern`), so `*`, `*/`, `[xy]` and `tpl*` keep every note while the imitation dropped the folder; and `gitignore: true` was not imitated at all. So `createIndexPage` now resolves `globby` from `<project>/quartz/util/glob.ts` (`createRequire`, then a dynamic `import()` — globby 16 is ESM-only with a `default` export condition, which `require.resolve` accepts), runs it as `build.ts` does (`**/*.*`, `ignore` from the config, `gitignore: true`) and lists every top-level `.md` and every top-level folder with at least one `.md` below — which is what folder-page makes a page of; a folder holding only images used to be listed and no longer is. The cwd is `content/` itself, not its resolved target, because globby also reads `.gitignore` files up to the root of the surrounding git repository. Measured on an esbuild bundle of `contentService` against globby from `gui-test/node_modules`, throwaway project with `x/a.md`, `x/sub/b.md`, `y/c.md`, `z.md`, `tpl/t.md`, `w/w.md`, `img/pic.png`, `Up.MD`: 26 patterns, 0 differences; a `.gitignore` in the vault agrees; a project `.gitignore` with `content/y/` and `tpl/` is ignored by both while the project is no repository and honoured by both once it is. Without `node_modules` (no build possible either) a walk restates fast-glob's two rules and reads no `.gitignore`; against the same 26 patterns it differs once, `{x,y}`, which fast-glob brace-expands into two static patterns first. In the built app with a throwaway profile (`content.createIndex`, patterns `y/*` and `*`, `.gitignore` with `tpl/` in a repository): the list held `x` only, Quartz globbed `x/a.md x/sub/b.md` — and since the fallback reads no `.gitignore`, that result came through globby. `sass` in `styleService` was already loaded from the project the same way (`createRequire(...)('sass')`); globby is the first such module loaded as ESM. What loading project code into the main process costs, and why it is accepted, is in `process-model-and-ipc.md` (review 2026-09-18, finding 2).

**The fallback says that it ran, and a broken globby is an error (2026-09-14, review 2026-09-18, finding 1).** The first version caught every failure to load globby in one `catch` and ran the walk silently. The fourteenth review measured the price at the built and the packaged app: a project whose `node_modules/globby/index.js` had a one-line syntax error got the fallback's list — `tpl x z` instead of Quartz's `z` for `ignorePatterns: ['./x', 'y/*']` and a `.gitignore` with `tpl/`, two links to pages the build does not make — with no word in the list, the dialog or the log; and the walk differed from globby in more cases than its comment said. Now the two failures are told apart by where they happen: `resolve('globby')` throwing `MODULE_NOT_FOUND` is "not installed" and takes the fallback, which `createIndex` reports as `listSource: 'fallback'` and the content card states under the folder after the dialog closes ("…ohne die Ausschlussregeln von Quartz, weil im Projekt noch kein npm install gelaufen ist"); a globby that resolves but does not load is an error with a sentence (`indexPageGlobbyBroken`) and no file is written. A dependency of globby missing counts as broken, not as absent — it throws from `import()`, not from `resolve()`. The walk also takes off what fast-glob and picomatch take off an ignore pattern: one leading `!` (`convertToPositivePattern`, not for `!(…)`) and a leading `./`. Measured, the walk cut out of `contentService.ts` into a `.mts` against globby 16.2.2 / fast-glob 3.3.3 from `gui-test`, 31 patterns over 14 entries: `./x`, `!x`, `!./x`, `/x`, `!(x)` now agree; 8 answers still differ — `{x,y}`, `foo\*` (Node's `matchesGlob` reads the backslash as a separator), `!!x`, and five where `matchesGlob` ignores case on macOS (`*.MD` drops `top.md`) — and apart from `{x,y}` and `!!x` each leaves out something the build makes rather than linking something it does not. At the built app, one fresh profile per project, dialog clicked: without `node_modules` the list is `tpl z top` and the card shows the sentence; with `node_modules` a symlink to a real one, `z top` and no sentence; with the broken globby no `index.md` and the error in the dialog ("…lässt sich nicht laden, deshalb ist keine Startseite angelegt (Unexpected end of input)…").

**The start screen's project grid follows its column, and the page says each thing once (2026-09-13).** A beta tester found the start screen unbalanced and repetitive; the maintainer found the project cards going two-column only on a wide window without their column getting any wider. Measured in the built app with a throwaway profile at 1280×800 and 1728×1000: the list was `2xl:grid-cols-2` - two cards from a 1536px *window* on - while the column is capped by `max-w-6xl` minus the 320px aside and stops growing at `xl`, around 808px; so 1280px showed one card per row 808px wide with almost nothing in it, and 1728px two in the same width. On the right, "Was du hier tun kannst" was four paragraphs and the tallest block on the screen: at 1728px the four project cards ended around 570px, the aside ran past 1000px. And four things were said twice: with no projects both actions stood in the button row *and* again in "Erste Schritte", with the primary the other way round; the card's introduction repeated "Was ist Quartz?" beside it; its steps 2 and 3 repeated the capability list; that list's four headings were the subtitle's three verbs. Changed (the user chose three of four proposals; hiding the site title when it equals the project name was not taken): the grid is `repeat(auto-fit, minmax(min(100%, 22rem), 1fr))` - `auto-fit`, so a lone card still spans the column rather than sitting beside an empty track, which is what the old `filtered.length > 1` guarded; the capability list is a title and a few words per area; "Erste Schritte" keeps its numbered steps and loses its introduction and its buttons, and with no projects the one button row puts "Neues Projekt erstellen" first. After: two cards per row at both widths, the aside ending at about the height of two card rows, no scroller on the page at either size with four projects, and with a "no quartz.config.yaml" card and a "folder not found" card (the one with "Ordner suchen…") at the narrower card width no button wider than its card. A one-line form of the capability list ("Einrichtung · Titel, …") was tried first and broke mid-word in the 320px aside ("Git-/Sync"), so it is two short lines.

**A running build is state of the main process, shown with a clock and a phase wherever the project is open (2026-09-13).** Asked for from the beta: `Example` builds long enough to look frozen. Measured first on a copy of `Example` in the built app: after "Jetzt bauen" the page said "Baue…", the Übersicht said nothing, and back on Vorschau & Build the button read "Jetzt bauen" again while the build ran - `building` was the page's own `useState` - and a second click then started a *second* `npm exec quartz build` into the same output directory beside the first (two pids, 40 s instead of 19). Now `buildService` holds a `BuildActivity` per project (`build` for "Jetzt bauen", `serve` for the dev server's first build, `rebuild` after a change), broadcast as `build:activityChanged` and readable through `build.activity`; `runBuild` returns the running build's promise to a second caller, and the IPC handler returns it before the output-directory guard asks its question again. The phase comes from the lines Quartz prints when stdout is not a terminal (QuartzLogger turns verbose then): "Parsing input files" → `parsing`, "Emitting files" → `emitting`, "Done processing" ends a `serve`, "Detected change, rebuilding..." starts a `rebuild` and "Done rebuilding in"/"Rebuild failed" end it; `preparing` covers npm, the frames and Quartz loading its plugins, and the listening port ends a `serve` if the done line never comes. `BuildActivityLine` shows "Build läuft seit 0:42 · schreibt die Seiten", ticking once a second and deliberately not in a live region. After, the same run: the line appeared on Vorschau & Build within a second and counted up, the Übersicht showed it with the build button disabled, back on the page the button still read "Baue…", a second `build.run` during the build left one pid and resolved with the first build's result, the build took 16 s, and when it ended the line went and the output card read "Zuletzt gebaut jetzt"; starting the dev server showed "Dev-Server baut seit …" from `preparing` to `emitting` until the state became `running`, and a changed note showed the rebuild line and cleared it when Quartz was done.

**Nachtrag (2026-09-16): ein Ausgabeordner hat einen Schreiber, und die zwei möglichen sind nicht gleich zu behandeln.** Das zwölfte Review hatte es nur beobachtet und offengelassen: Ein einmaliger Build und der Dev-Server schreiben zugleich in dasselbe `public/` — im Mitschnitt 16 Dateien eines Server-Neubaus mitten im „Emitting files“ des Builds. `quartz build` leert seinen Ausgabeordner zuerst und schreibt ihn dann Datei für Datei, der Server baut bei jeder Änderung im Content-Ordner neu; was danach im Ordner liegt, ist von keinem der beiden. Die zwei Richtungen bekommen **zwei verschiedene Antworten**, weil nur eine von ihnen warten kann: Ein Klick auf „Jetzt bauen“ wird **abgelehnt**, solange der Dev-Server dieses Projekts in denselben Ordner schreibt (`assertOutputFree()`, geprüft im Handler *vor* der Ordner-Rückfrage und noch einmal in `runBuild`, weil dazwischen ein Lesevorgang und womöglich ein Dialog liegen), und die Meldung nennt die zwei Wege — Server beenden oder anderen Ausgabeordner. Ein Klick auf „Starten“ dagegen **wartet** auf den laufenden Build und startet danach; das braucht keinen zweiten Klick, und damit ein Start, der eine halbe Minute nichts tut, nicht wie ein gescheiterter aussieht, schreibt er den Grund in die Server-Konsole. Ein Server im Zustand `stopping` zählt weiter als Schreiber: Das Signal ist gesendet, der Prozess schreibt bis er weg ist. Was der Wächter **nicht** sieht, ist ein Server aus einer früheren Sitzung, den der Nutzer beim Beenden hat weiterlaufen lassen — der gehört keinem Eintrag in `runningServers`, und ihn zu finden hieße, bei jedem Klick die Ports abzusuchen; die Vorschau-Seite zeigt solche Server getrennt. Gemessen an der gebauten App mit Wegwerf-Profil gegen eine Kopie von `navigations-testprojekt`, Server auf 8099/3099, vorher und nachher: vorher lief der Build bei laufendem Server ohne Widerspruch durch (4,9 s) und ein Serverstart schob sich nach 38 ms in einen laufenden Build; nachher wird der Build nach 3 ms mit dem Satz abgelehnt, ein Build in einen anderen Ordner läuft weiter durch (5,1 s, Server unberührt `running`), und der Serverstart wartet 4,1 s auf das Ende des Builds, mit der Zeile „Es läuft ein Build nach … Der Server startet, sobald er fertig ist.“ in der Konsole.

**Nachtrag (2026-09-16, zwölftes Review): one activity, two writers, and a join that did not look at the folder.** Four of the sentences above no longer hold, and all four were measured wrong in the built app with a throwaway profile, a copy of `gui-test` and its dev server on 8099/3099, events recorded in the renderer. (1) `followQuartzOutput` was called by the one-off build's stdout and by the dev server's log tail without knowing which: with the dev server running, a note changed in the third second of `quartz build` made the server's "Detected change, rebuilding..." replace the `build` activity and its "Done rebuilding" clear it — the build ran on for 3.3 of its 6.9 s with no line and no lock, and the build's own `.finally` found nothing to clear. Now the follower takes its source: the build moves only a `build` activity, the server nothing while a `build` stands. After: rebuild at 21.1 s, `build`/`emitting` stays until 24.5 s, cleared by the build's "Done processing". (2) "Rebuild failed:" is written with `console.error` (`build.ts:174`), i.e. into the `.err.log` whose tail did not call the follower; with both lines appended by hand to the running server's logs the `rebuild` activity stayed before and was cleared after. Both tails go through the follower now. (3) Quartz has a second rebuild: the CLI watcher on the config, `*.scss`, `*.ts` and `quartz/static` says "Detected a source code change, doing a hard rebuild..." and then prints the lines of a first build, ending on "Done processing" — and every save in this app is that kind. With the config saved unchanged: no activity for 4.8 s before; now `preparing` 10.8 s, `parsing` 11.0 s, `emitting` 11.6 s, cleared 15.9 s. The soft rebuild never prints "Emitting files" (read in the real `dev-server-28737.out.log`: "Filtered out" goes straight to "Emitted"), so "Filtered out N files" moves the phase too; a changed note now shows `parsing` → `emitting` → cleared in 0.2 s. (4) The handler returned the running build *before* comparing output directories, and the Übersicht builds into `public/` while Vorschau & Build uses the page's own choice: three simultaneous `build.run` (none, `public`, `dist`) all resolved `success: true`, and `dist` did not exist. `runningBuilds` now keeps the resolved directory, `joinRunningBuild()` joins only the same one (compared with `relative()`), and a different one is refused with a sentence naming both; checked in the handler and again in `runBuild`, because a disk read and possibly a dialog lie between them. After: the first two resolved, the third was refused.

**Nachtrag (2026-09-16, sechzehntes Review): eine Wartezeit, die niemand sieht, ist eine Einladung zum zweiten Klick — und ein Map-Eintrag unter einer Projekt-ID trägt so wenig über die Zeit wie eine PID.** Der wartende Serverstart des Absatzes darüber war für die Oberfläche unsichtbar: Zwischen dem Klick und dem Spawn stand der Status auf `stopped` und damit der Knopf auf „Starten“, und zwar die ganze Dauer des Builds statt der 38 ms von `refreshAuthoredFrames`. Gemessen an der gebauten App mit Wegwerf-Profil gegen eine Kopie von `navigations-testprojekt`: Zwei Klicks kamen an der Sperre vorbei, beide spawnten, der zweite starb am Port — und sein `exit`-Handler löschte den Eintrag des ersten samt seinem Satz in `running-servers.json`; danach sagte die Seite „Fehler“ und bot „Starten“, während der erste Server lief. Seither trägt der wartende Start einen eigenen Eintrag (`pendingStarts`, Status `starting`, sofort per `emitStatus` gesagt), ein zweiter Klick bekommt denselben Eintrag zurück statt eines zweiten Spawns, und geräumt wird ein Eintrag nur, wenn `runningServers.get(id)?.process` noch dieses Kind ist (`forgetServer()`). Nachgemessen im siebzehnten Review, `build.run` bei 0, `server.start` bei 0,80 s, ein zweites bei 1,40 s: 803 ms `starting` ohne PID, 1406 ms antwortet der zweite Start mit demselben Eintrag, 7211 ms Build fertig, 7260 ms antwortet der erste Start mit PID 81742, 12287 ms `running` — ein Prozess, ein Eintrag. „Stoppen“ während des Wartens antwortet nach 1 ms mit `stopped` und bricht es ab; ein erneuter Start wartet und spawnt.

**Nachtrag (2026-09-16, siebzehntes Review): `startedAt` ist die Spawnzeit, und die Übersicht hat auch während des Wartens einen Ausgang.** Zwei Folgen des Eintrags oben, beide erst an der gebauten App sichtbar. (1) `startServer` setzte `startedAt` beim Klick, `spawnServer` setzte nur die PID — seit ein Start auf einen Build wartet, liegen die zwei eine ganze Builddauer auseinander, und beide Seiten lesen die Zeit im Zustand `running` als „Gestartet vor …“: gemessen „vor 10 Sekunden“ für einen Prozess, der 6,5 s alt war, und auf `Example` sind es 16 bis 19 s. Gesetzt wird sie jetzt dort, wo die PID entsteht; am Bündel von `buildService` mit einer npx-Attrappe (Build 4 s, Klick nach 0,8 s) ist der Abstand zur Klickzeit von 1 ms auf 3251 ms gewandert, also auf den Rest des Builds. Während des Wartens zeigt keine Seite eine Zeit, dort bleibt die Klickzeit das Einzige, was es zu sagen gibt. (2) Auf der Übersicht stand „Stoppen“ nur bei `running`; bei `starting` standen „Starten“ und „Neu starten“ da und beide deaktiviert. Auf *Vorschau & Build* lag der Abbruch immer richtig, auf der Übersicht lag er nie dort — nur dauerte `starting` vorher Sekunden und jetzt einen Build, auf der Seite, die den Start angeboten hatte. Gemessen mit Build bei 0 s, Start bei 0,8 s, DOM bei 2,3 s: vorher `[Starten disabled=true, Neu starten disabled=true]`, nachher `[Stoppen disabled=false, Neu starten disabled=true]` — und der Knopf taugt als Abbruch (Klick bei 2,0 s, `stopped` nach 1 ms, nach dem Buildende bei 7,5 s kein Spawn). Die drei Server-Aufrufe der Übersicht laufen dafür nicht mehr unter `withBusy`: Die Antwort kommt erst mit dem Spawn, also nach dem Build, und hätte alle drei Knöpfe genau so lange gesperrt. Welcher Knopf dasteht, sagt das Abonnement.

**Nachtrag (2026-09-17, einundzwanzigstes Review): ein Rat gehört zu dem Zustand, den er meint — und eine Seite, die nicht lädt, sagt warum.** Zwei Ränder derselben Sorte, beide an der gebauten App mit Wegwerf-Profil und einer Projektkopie gemessen.

Der Nachsatz „Existiert die Datei im Projektordner? Ein neu erstelltes Projekt braucht dafür einen erfolgreich durchgelaufenen Setup-Assistenten“ stand auf der Konfigurations- und der Layout-Seite unter **jedem** Lesefehler — seit `readConfig` drei Arten von unlesbar kennt also auch unter einem, der gerade gesagt hat, dass die Datei da ist und was in ihr steht. Der Rat gehört zu einem Zustand, also sagt ihn der Zustand: `readConfig` fängt `ENOENT` ab und antwortet mit `configMissing`; der Nachsatz im Renderer fällt weg. Gemessen in drei Zuständen: fehlende Datei → „quartz.config.yaml liegt nicht im Projektordner. Ein neu erstelltes Projekt braucht dafür einen erfolgreich durchgelaufenen Setup-Assistenten.“; Syntaxfehler → „…lässt sich nicht als YAML lesen: Missing closing "quote at line 5, column 1:…“; heile Datei unverändert.

Und die Stile-Seite blieb bei einem gescheiterten `config:get` für immer auf „Lade…“ stehen — ohne Kopf, ohne Reiter —, weil ihre zwei tragenden Lesevorgänge kein `catch` hatten und nur der globale Handler einen Toast in die Ecke stellte. Sie haben jetzt eines und die Seite einen Fehlerzustand, wie Konfiguration und Layout ihn schon hatten; die zwei übrigen Lesevorgänge dürfen weiter scheitern, ohne die Seite anzuhalten. Der Satz geht durch `formatIpcError`, wie an jeder anderen Stelle, die einen IPC-Fehler in der Seite zeigt — roh stand davor „Error invoking remote method 'config:get': Error: …“.

**Nicht behoben, weil es keiner ist:** Dass die App in einem Projekt ohne `.gitignore` eine anlegt, ist die Entscheidung aus `projectDirs.ts` — `.quartz-gui/` liegt im Repo des Nutzers, und `quartz sync` pusht alles, also stünden die Config-Kopien mit ihren Analytics-Schlüsseln in einem meist öffentlichen Repo. Nachgesehen: in einem echten Quartz-Projekt gibt es die Datei längst und die App hängt eine Zeile an (`navigations-testprojekt`, Zeile 17); nur in den Attrappen-Szenen dieser Runde, deren Upstream keine hat, entsteht eine neue mit dem Kommentar „# Arbeitsverzeichnis von QuartzControl“. Dass dieser Kommentar auch in einer englischen Installation deutsch ist, ist dabei aufgefallen und nicht mit erledigt.

**Nachtrag (2026-09-17, zweiundzwanzigstes Review): die Stile-Seite war eine von dreien.** Dieselbe Frage, an den übrigen Seiten gestellt: Wer liest beim Mount ohne `catch` und zeigt ohne die Antwort „Lade…“? Der Reiter *Übersetzungen* und `ProjectLayout`. Beim Reiter hatte der Fall einen Rand, der die Seite gar nicht hätte anhalten dürfen — `config.get` entscheidet dort nur, welche Sprache vorausgewählt ist; scheiterte es, blieb `code` ungesetzt, der Effekt darunter lief nie, und der Eintragsblock sagte „Lade…“ neben einer Sprachliste, die geladen hatte. Der Aufruf fällt jetzt auf die erste Sprache zurück, die zwei tragenden Lesevorgänge bekommen einen Fehlerzustand. `ProjectLayout` hat **zwei** Wege, nicht zu antworten, und beide sahen gleich aus: ein Fehler und ein `null` — ein Projekt, das die Liste nicht mehr hat, wie es eine gemerkte Route nach dem Entfernen sieht. Das ist der eine Bildschirm ohne Seitenleiste und damit ohne Ausgang; er sagt es jetzt und trägt einen Link zur Projektliste. Gemessen an der gebauten App: vorher „Lade…“ bzw. „Lade Projekt…“, nachher die Einträge bzw. „Dieses Projekt steht nicht mehr in der Liste.“ mit „Alle Projekte“.

**Nachtrag (2026-09-17, dreiundzwanzigstes Review): der Reiter *Übersetzungen* gab beiden Lesevorgängen dieselbe Antwort.** Die Regel des Fixes oben — „die Seite ist darauf angewiesen“ ist eine Frage je Lesevorgang — gilt eine Ebene tiefer noch einmal. Ohne die Liste gibt es nichts zu zeigen; ohne die Einträge *einer* Sprache gibt es noch die anderen dreißig, und der Wähler darüber ist der Weg zu ihnen. Der frühe `return` ersetzte den ganzen Reiter, die gewählte Sprache ist sticky, also führte auch Verlassen und Wiederkommen in denselben Bildschirm — bis zum Neustart der App war keine Sprache mehr erreichbar. Gemessen an der gebauten App mit `de-DE Kopie.ts` im Locale-Ordner (so nennt der Finder ein Duplikat; `listLocales` führte jede `*.ts`, `s.localeCode` weist den Namen ab):

    vorher   Wähler auf „de-DE Kopie“: kein `<select>` mehr im Reiter, 0 Zeilen
             Reiter verlassen und zurück: derselbe Bildschirm
             Datei gelöscht, zurück: noch einmal derselbe Bildschirm
    nachher  Wähler steht (31 Optionen), darunter „Die Sprache „de-DE Kopie“ ließ sich nicht
             laden.“ samt zod-Satz, 0 Zeilen; Wechsel auf en-US: Fehler weg, 8 Zeilen

Geräumt wird der Zustand bei jedem Versuch, nicht nur beim Wechsel — damit ist auch der Besuch nach dem Entfernen der Datei wieder eine frische Frage. **Und die Datei kommt gar nicht mehr in den Wähler:** `listLocales` führte jede `*.ts`, der Kanal daneben wies alles ab, was kein Locale-Code ist, und der Satz dazu schrieb den Fehlschlag der App zu statt der Datei. Beide Seiten fragen jetzt dieselbe Funktion (`isLocaleCode` in `shared/ipc-contract.ts`); das zod-Schema ist ihr `refine`. Gemessen: vorher 31 Optionen einschließlich „de-DE Kopie“, nachher 30 ohne sie.
